...
export default function EditCourseModal({ isOpen, onClose, course, actionData }) {
  // Form state
  ...
  const validateForm = () => {
    ...
    if (!classroomLink.trim()) {
      ...
      try {
        ...
        const url = classroomLink.match(/^https?:\/\//) ? classroomLink : `https://${classroomLink}`;
        new URL(url);
      } catch (e) {
  ...
  // Populate form fields when modal opens or course changes
  useEffect(() => {
    if (isOpen && course) {
      setName(course.name || "");
      setClassroomLink(course.classroom_link || "");
      setCost(parseInt(course.cost) || 0); // Asegurar que sea número
      
      // Format dates if they exist
      if (course.start_date) {
        const formattedStartDate = new Date(course.start_date).toISOString().split('T')[0];
      ...
      if (course.end_date) {
        const formattedEndDate = new Date(course.end_date).toISOString().split('T')[0];
        setEndDate(formattedEndDate);
      } else {
        setEndDate("");
      }
      
      setStatus(course.status?.toString() || "0");
      setErrors({});
      setIsTouched({});
    }
  }, [isOpen, course]);
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <Form method="post" className="p-5" onSubmit={handleSubmit}>
          ...
          <div className="space-y-5">
            ...
            {actionData?.error && actionData?._action === "update" && (
            ...
            {/* Success message */}
            {actionData?.success && actionData?._action === "update" && (
              <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded">
                <p className="text-sm text-green-700">Curso actualizado con éxito</p>
              </div>
            )}
