...
import { Search, Users, Shield, Globe, AlertCircle, ChevronDown, ChevronUp, X, CheckCircle } from "lucide-react";
import JoinCommunityModal from "./_searchcommunitiesmodals/dashboard-user-joincommunitymodal"
...
export async function loader({ request }) {
  ...
  try {
    ...
    if (userCommunitiesResponse.ok) {
      ...
      console.error("Error fetching user communities:", await userCommunitiesResponse.text());
    }
    ...
    if (pendingTransactionsResponse.ok) {
      ...
      console.error("Error fetching pending transactions:", await pendingTransactionsResponse.text());
    }
    
    // Filter out communities with visibility=0 (hidden)
    const visibleCommunities = communitiesData.data ? 
      communitiesData.data.filter(community => community.visibility !== 0) : 
      [];
...
export default function SearchCommunities() {
  ...
  const filteredCommunities = sortedCommunities.filter(community => {
    ...
    // We're already filtering hidden communities in the loader, so we don't need to check visibility here
  ...
  const isUserMember = (communityId) => {
    // userCommunities now contains objects with just an id property
